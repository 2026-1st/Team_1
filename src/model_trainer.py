...
class ModelTrainer:
    ...
    def _load_and_preprocess(self):
        ...
        # CSV에 없는 모델용 파생 피처 생성
        df['trend_lag1'] = df.groupby('ticker')['weighted_trend'].shift(1)
        df['trend_lag3_mean'] = df.groupby('ticker')['weighted_trend'].transform(
            lambda x: x.rolling(3).mean()
        )
        df['trend_lag7_mean'] = df.groupby('ticker')['weighted_trend'].transform(
        ...
        df = df.dropna(subset=self.features + [self.target]).reset_index(drop=True)
    ...
    def evaluate_model(self, model, X_test, y_test, model_name="Model"):
        ...
        if roc_auc is not None:
            ...
            print("ROC-AUC  : 계산 불가 (predict_proba 또는 decision_function 없음)")
        
        print("\n[Classification Report]")
        print(classification_report(y_test, y_pred))
        
        # Confusion Matrix: Precision, Recall, F1 해석에 필요한 오분류 구조 확인
        ...
        # ROC Curve: 분류 임계값 변화에 따른 TPR/FPR 확인
